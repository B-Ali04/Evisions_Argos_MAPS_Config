  ...
  FROM
      ...
      LEFT OUTER JOIN SFRTHST SFRTHST ON SFRTHST.SFRTHST_PIDM = SGBSTDN.SGBSTDN_PIDM
            AND SFRTHST_SURROGATE_ID = (SELECT MAX(SFRTHST_SURROGATE_ID)
                                        FROM   SFRTHST THST
                                        WHERE  THST.SFRTHST_PIDM = SGBSTDN.SGBSTDN_PIDM
                                        AND    THST.SFRTHST_TERM_CODE = SGBSTDN_TERM_CODE_EFF)
  ),
...
(
  ...
  WHERE
      SFRWDRL.SFRWDRL_TERM_CODE = (SELECT MAX(SFRWDRL_TERM_CODE)
                                  FROM    SFRWDRL WDRL
                                  WHERE   WDRL.SFRWDRL_PIDM = SFRWDRL.SFRWDRL_PIDM
...
(
  ...
  WHERE
      ...
      AND SHRTGPA.SHRTGPA_TERM_CODE = (SELECT MAX(SHRTGPA_TERM_CODE)
                                      FROM    SHRTGPA TTTGPA
                                      WHERE   TTTGPA.SHRTGPA_PIDM = SHRTGPA.SHRTGPA_PIDM
                                      AND     TTTGPA.SHRTGPA_GPA_TYPE_IND  = SHRTGPA.SHRTGPA_GPA_TYPE_IND
                                      AND     TTTGPA.SHRTGPA_LEVL_CODE = SHRTGPA.SHRTGPA_LEVL_CODE
                                      AND     TTTGPA.SHRTGPA_TERM_CODE <= :select_term_code.STVTERM_CODE)
  ),
  ...
  FROM
      ...
      LEFT OUTER JOIN SFRSTCA SFRSTCA ON SFRSTCA.SFRSTCA_PIDM = SFRSTCR_PIDM
            ...
            AND SFRSTCA.SFRSTCA_SEQ_NUMBER = (SELECT MAX(SFRSTCA_SEQ_NUMBER)
                                             FROM    SFRSTCA STCA
                                             WHERE   STCA.SFRSTCA_PIDM = SFRSTCA.SFRSTCA_PIDM
                                             AND     STCA.SFRSTCA_TERM_CODE = SFRSTCA.SFRSTCA_TERM_CODE
                                             AND     STCA.SFRSTCA_CRN = SFRSTCA.SFRSTCA_CRN)
