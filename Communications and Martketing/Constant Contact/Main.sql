WITH main_1 AS
 (SELECT lower(goremal_email_address) email,
         ...
         spriden_first_name fname , spriden_last_name lname,
         a.sgbstdn_pidm pidm
...
main_esf as
 (select distinct lower(a.goremal_email_address) email,
                  main_1.levl levl,
                  main_1.fname fname, main_1.lname lname
...
main_2 as
 (
  ...
  select 'aelombard@esf.edu', 'EMP', 'Anne','Lombard'
    from dual
    -- Anne Lombard is no longer affilated with SUNY ESF
...
,
main_pop as
 (select main_1.email, main_1.levl, main_1.fname, main_1.lname
    from main_1
