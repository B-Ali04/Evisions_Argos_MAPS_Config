...
and (case WHEN :chkReg=1 then
          CASE WHEN st.sgbstdn_pidm in (select DISTINCT sfrstcr_pidm from sfrstcr where sfrstcr_TERM_CODE >= :selMinTerm.CODE and sfrstcr_term_code <= :selMaxTerm.CODE and SFRSTCR_RSTS_CODE='RE' and sfrstcr_crn <> 90012) THEN 1 ELSE 0 END
          ELSE 1 END)=1
and (case WHEN :chkUnReg=1 then
          CASE WHEN st.sgbstdn_pidm not in (select DISTINCT sfrstcr_pidm from sfrstcr where sfrstcr_TERM_CODE >= :selMinTerm.CODE and sfrstcr_term_code <= :selMaxTerm.CODE and SFRSTCR_RSTS_CODE='RE' and sfrstcr_crn <> 90012) THEN 1 ELSE 0 END
          ELSE 1 END)=1
)
