...
-- Reporting Use Only
    ...
    (SELECT STVTERM_DESC
    FROM    STVTERM TERM
    WHERE   TERM.STVTERM_START_DATE >= :select_start_date
    AND     TERM.STVTERM_END_DATE <= :select_end_date
    AND     STVTERM_CODE = (SELECT MAX(STVTERM_CODE)
                           FROM    STVTERM TERM
                           WHERE   TERM.STVTERM_START_DATE >= :select_start_date
                           AND     TERM.STVTERM_END_DATE <= :select_end_date)) as TERM_DESC,
...
-- Commencement
  ...
  -- JOSTENS
    ...
    f_format_name(SPRIDEN_PIDM, 'FMIL') as Printed_Name,
    SHRDGMR_DEGC_DESC as Degree,
    case
      when SHRDGMR_DEGC_CODE = 'AAS' then '('||SHRDGMR_MAJR_DESC ||')'
      when SHRDGMR_DEGC_CODE not in('AAS') then NULL
        else NULL
          end as Major,
    SHRDGIH_HONR_DESC as Honors
